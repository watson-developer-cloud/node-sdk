...
function createServiceAPI(serviceName) {
  return function(user_options) {
    ...
    var service = {};
    if (user_options.use_vcap_services)
      service = bluemix.serviceStarsWith(serviceName);

    // Remove the url from Bluemix. This will be removed in the future
    if (service.credentials)
      delete service.credentials.url;

    var options = extend(service.credentials, user_options);
