...
function BaseService(user_options) {
  ...
  this._options = extend({qs: {}}, this.serviceDefaults, options);
}

BaseService.prototype.initCredentials = function(options) {
  if (options.token) {
    options.qs = options.qs || {};
    options.qs['watson-token'] = options.token;
    return options;
  }
  ...
  if (options.use_vcap_services !== false) {
    var credentials = this.getCredentialsFromEnvironment(this.name);
    options = extend({}, credentials, options);
  }
 ...
 *
 * @todo: consider also supporting simpler env property names such as SPEECH_TO_TEXT_USERNAME & SPEECH_TO_TEXT_PASSWORD
 *
 * @private
 * @returns {{}}
 */
BaseService.prototype.getCredentialsFromEnvironment = function(vcap_services_name) {
