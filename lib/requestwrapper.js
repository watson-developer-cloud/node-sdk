...
function createRequest(parameters, callback) {
  var missingParams = null,
    options = extend({}, parameters.default_options, parameters.options),
    path    = options.path,
  ...
  // Missing parameters
  missingParams = helper.getMissingParams(extend({}, qs, body, form, path),
  ...
  if(options.url.charAt(0) === '/'){
      options.url = parameters.default_options.url + options.url;
  }

  //console.log('requests.options:',options);
