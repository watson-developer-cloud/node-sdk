...
class RecognizeStream extends Duplex {
  ...
  initialize() {
    ...
    const queryString = Object.keys(queryParams)
      .map(function(key) {
        ...
        ); // our server chokes if the token is correctly url-encoded
      })
      .join('&');
    ...
    socket.onerror = function(error: Error): void {
    ...
    this.socket.onclose = function(e): void {
      self.listening = false;
      self.push(null);
      /**
     ...
     */
    function emitError(msg, frame, err?): void {
      if (err) {
    ...
    socket.onmessage = function(frame: { data: string }): void {
  ...
  private static getContentType(buffer: Buffer): string {
