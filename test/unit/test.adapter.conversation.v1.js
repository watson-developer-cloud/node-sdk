...
describe('conversation-v1', function() {
  ...
  describe('updateSynonym()', function() {
    ...
    it('should generate a valid payload', function() {
      ...
      assert.equal(req.uri.href, service.url + paths.synonyms + '/' + synParams2.old_synonym + '?version=' + service.version_date);
      assert.equal(req.method, 'POST');
    });
  });
  ...
  describe('updateIntent()', function() {
    ...
    it('should generate a valid payload', function() {
      ...
      assert.equal(req.uri.href, service.url + paths.intents + '/' + intentParams.old_intent + '?version=' + service.version_date);
      assert.equal(req.method, 'POST');
    });
  });
  ...
  describe('updateEntity()', function() {
    ...
    it('should generate a valid payload', function() {
      ...
      assert.equal(req.uri.href, service.url + paths.entities + '/' + reqPayload.old_entity + '?version=' + service.version_date);
      assert.equal(req.method, 'POST');
    });
  });

  describe('updateValue()', function() {
    const reqPayload = { entity: payload2.entity , value: 'val', old_value: 'val_old'};
    const params = extend({}, reqPayload, payload);
    ...
    it('should generate a valid payload', function() {
      ...
      assert.equal(req.uri.href, service.url + paths.values + '/' + reqPayload.old_value + '?version=' + service.version_date);
      assert.equal(req.method, 'POST');
    });
  });
  ...
  describe('updateExample()', function() {
    const reqPayload = { text: 'text' , old_text: 'oldtext', intent: payload2.intent};
    const params = extend({}, reqPayload, payload);
    ...
    it('should generate a valid payload', function() {
      ...
      assert.equal(req.uri.href, service.url + paths.examples + '/' + reqPayload.old_text + '?version=' + service.version_date);
      assert.equal(req.method, 'POST');
    });
  });

  describe('updateDialogNode()', function() {
    const reqPayload = { dialog_node: 'foo' , new_dialog_node: 'foo_new', old_dialog_node: 'foo_old'};
    const params = extend({}, reqPayload, payload);
