...
describe('discovery-v1', function() {
  ...
  const paths = {
    ...
    fields: '/v1/environments/env-guid/collections/col-guid/fields',
  ...
  describe('discovery versions', function() {
    ...
    describe(`discovery(version=${service.version})`, function() {
      ...
      it('should create a collection in an environment', function() {
        ...
        assert.equal(req.uri.href, service.url + paths.collections + '?version=' + service.version);
        assert.equal(req.method, 'POST');
      });
      ...
      it('should get information about a specific collections fields', function() {
        ...
        assert.equal(req.uri.href, service.url + paths.fields + '?version=' + service.version);
        assert.equal(req.method, 'GET');
      });
        ...
        const req = discovery.updateConfiguration(
          {
            ...
            // file is a JSON Object not a file
            file: JSON.parse(
              fs.readFileSync(path.join(__dirname, '../resources/discovery-sampleUpdateConf.json'))
            ),
          },
          noop
        );
