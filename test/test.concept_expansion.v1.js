...
describe('concept_expansion', function() {
  ...
  var service_response = {
    return_seeds: [
      { prevalence: 1, result: 'zZzStarzZz' },
      { prevalence: 2, result: 'zZzPluszZz' }
    ]
  };
  ...
  before(function(){
    ...
    .reply(200,{jobid:'2014'})
    // Get status
    .get('/v1/status?jobid=2014')
    .reply(200,{state:'D'})
    .put('/v1/result',{jobid:2014})
  ...
  it('should check for missing seeds', function() {
    var params = {dataset: payload.dataset, label: payload.label};
    concept_expansion.expand(params, missingParameter);
  });

  it('should check for missing dataset', function() {
    var params = {seeds: payload.seeds, label: payload.label};
    concept_expansion.expand(params, missingParameter);
  });
  it('should check for missing label', function() {
    var params = {seeds: payload.seeds, dataset: payload.dataset};
    concept_expansion.expand(params, missingParameter);
  });
