...
const document_conversion = new DocumentConversionV1({
...
document_conversion.index(
  {
    metadata: {
      metadata: [{ name: 'id', value: '1' }, { name: 'SomeMetadataName', value: 'SomeMetadataValue' }]
    },
    config: {
      retrieve_and_rank: {
        dry_run: true
...
document_conversion.index(
  {
    ...
    config: {
      ...
      retrieve_and_rank: {
        dry_run: true,
        fields: {
          mappings: [{ from: 'Author', to: 'Created By' }, { from: 'Date Created', to: 'Created On' }],
...
document_conversion.index(
  {
    file: fs.createReadStream(__dirname + '/resources/example.html'),
    metadata: {
      metadata: [{ name: 'id', value: '3' }, { name: 'SomeMetadataName', value: 'SomeMetadataValue' }]
    },
    config: {
      convert_document: {
        normalized_html: {
      ...
      retrieve_and_rank: {
        ...
        fields: {
          mappings: [{ from: 'SomeMetadataName', to: 'Created By' }]
        }
      }
    }
  },
